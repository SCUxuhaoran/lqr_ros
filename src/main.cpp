...
#include<sys/time.h>
#include<Eigen/Eigen>
#include"my_lqr/cubic_spline.h"
#include"my_lqr/motion_model.h"
#include"my_lqr/cpprobotics_types.h"
#include"my_lqr/lqr_node.h"
...
cv::Point2i cv_offset(
  float x, float y, int image_width=800, int image_height=800){
  cv::Point2i output;
...
void update (State& state, float a, float delta){
  ...
  float right_speed = 100;

  if (delta >= MAX_STEER) delta = MAX_STEER;
  if (delta <= - MAX_STEER) delta = - MAX_STEER;

  if(delta > 0) {
    left_speed = 80*std::cos(delta);
  }
  if(delta < 0) {
    right_speed = 80*std::cos(delta);
  }
  ...
  std::cout << "X: " << state.x << "  Y: " << state.y << "  Yaw: " << state.yaw << std::endl;
  //cout << "[XUHAORAN] position: (" << state.x << ", " << state.y << ") [over] angle:" << theta << endl; 
      //my control
  //run(100, left_speed, right_speed);
  //state.x = (visualData.getVisualY())/100;
  //state.y = (400-visualData.getVisualX())/100;
  //state.yaw = (180-theta)/180*M_PI;
  //state.v = state.v + a * DT;
  //cout << "[XUHAORAN] position: (" << state.x << ", " << state.y << ") [over] angle:" << state.yaw << endl; 
      //my simulate
  //state.x = state.x + state.v * std::cos(state.yaw) * 0.1;
  //state.y = state.y + state.v * std::sin(state.yaw) * 0.1;
  //state.v = state.v + a * DT;
};
...
void closed_loop_prediction(Vec_f cx, Vec_f cy, Vec_f cyaw, Vec_f ck, Vec_f speed_profile, Poi_f goal){
  ...
  int count = 0;
  cv::Mat bg(800, 800, CV_8UC3, cv::Scalar(255, 255, 255));
...
int main(){
  ...
  Vec_f wx({1.0, 1.0, 1.0, 1.3, 2.0, 2.7, 3.0, 3.0, 3.0, 2.7, 2.0, 1.3});
  Vec_f wy({1.0, 2.0, 3.0, 3.7, 4.0, 3.7, 3.0, 2.0, 1.0, 0.3, 0.0, 0.3});
  //Vec_f wx({0.0, 1.0, 2.0, 2.5, 2.0, 1.0, 0.5, 1.0, 2.0, 2.5, 2.0, 1.0, 0.0, 0.0, 0.0});
  //Vec_f wy({0.0, 0.0, 0.0, 0.5, 1.0, 1.0, 1.5, 2.0, 2.0, 2.5, 3.0, 3.0, 3.0, 2.0, 1.0});
